...
export abstract class InternalClient extends EventEmitter {
        ...
        if (this._userRegisteredMethodListener) {
          ...
          this._enableMethods((err) => {
            if (err) {
              ...
              this.emit('preDisconnect', "re-enabling methods failed.");
        ...
        if (this._twin && this._twin.userRegisteredDesiredPropertiesListener) {
          ...
          this._twin.enableTwinDesiredPropertiesUpdates((err) => {
            if (err) {
              ...
              this.emit('preDisconnect', "re-enabling twin failed.");
              this.emit('disconnect', new results.Disconnected(err));
            }
          });
        }
      } else {
        this.emit('preDisconnect',"NoRetry");
        this.emit('disconnect', new results.Disconnected(err));
      }
    };
  ...
  setMaxOperationTimeout(timeout: number): void{
    if (false === Number.isInteger(timeout)){
      throw new TypeError('timeout is not integer value.');
    }
    if (timeout < 0){
      throw new Error('timeout is not zero and not a natural number.');
    }
    this._maxOperationTimeout = timeout;
  }
